...
export default function List() {
    ...
    useEffect(() => {
        ...
        let mounted = true
        api
            .getAdotanteCompatibility(id)
            .then((rels: any[]) => {
                if (!mounted) return
                const map: Record<number, number> = {}
                for (const r of rels) {
                    if (r.idPet !== undefined && r.compatibilidade !== undefined) {
                        map[r.idPet] = r.compatibilidade
                    }
                }
                setCompatMap(map)
            })
    ...
    const filteredPets = useMemo(() => {
        return allPets.filter(pet => {
            // 🔒 NÃO listar pets que já têm adotante vinculado
            const hasAdopter =
                (pet as any).adopterId != null && (pet as any).adopterId !== undefined
                    ? true
                    : (pet as any).idTutorAdotante != null &&
                    (pet as any).idTutorAdotante !== undefined

            if (hasAdopter) return false
            ...
            if (filters.especie !== 'Todos') {
                const especieMatch =
                    filters.especie === 'Cachorro'
                        ? pet.type === 'dog' ||
                        pet.tags.some(t => t.toLowerCase().includes('cachorro'))
                        : pet.type === 'cat' ||
                        pet.tags.some(t => t.toLowerCase().includes('gato'))
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={e =>
                                        handleFilterChange('vacinado', e.target.value)
                                    }
                                    className="w-full h-10 bg-[#F5E6C3] border-2 border-[#5C4A1F] rounded-xl text-[#5C4A1F] text-sm text-center font-medium focus:ring-2 focus:ring-[#F5B563] focus:border-[#F5B563] transition-all"
                                >
                                    <option value="Todos">Todos</option>
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={e =>
                                        handleFilterChange('castrado', e.target.value)
                                    }
                                    className="w-full h-10 bg-[#F5E6C3] border-2 border-[#5C4A1F] rounded-xl text-[#5C4A1F] text-sm text-center font-medium focus:ring-2 focus:ring-[#F5B563] focus:border-[#F5B563] transition-all"
                                >
                                    <option value="Todos">Todos</option>
